defmodule Redexor.Admins do
  ...
  alias Redexor.Admins.{Admin, AdminNotifier, AdminToken}
  ...
  @doc """
  Updates the admin email.
  """
  def update_admin_email(admin, password, attrs) do
    with {:ok, _applied_admin} <- apply_admin_email(admin, password, attrs) do
      admin
      |> change_admin_email(attrs)
      |> Repo.update()
    end
  end
