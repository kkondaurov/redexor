defmodule Redexor.RequestHandlerTest do
  ...
  describe "Test servers, methods and paths" do
    ...
    test "handle/4 returns error givem enabled server, but method and path of a disabled rdx_route", %{user: user, server: server, rdx_route: rdx_route} do
      {:ok, rdx_route} = RdxRoutes.update_rdx_route(user, rdx_route, %{enabled: false})
      assert %ApiResponse{code: 404} = RequestHandler.handle(server.id, rdx_route.method, rdx_route.path, %{}, %{})
    end
    ...
    test "handle/4 returns error given disabled server, and existing method and path", %{user: user, server: server, rdx_route: rdx_route} do
    ...
    test "handle/4 returns error given non-existent server", %{rdx_route: rdx_route} do
      ...
      assert %ApiResponse{code: 404} = RequestHandler.handle(fake_server_id, rdx_route.method, rdx_route.path, %{}, %{})
    end
  end
end
