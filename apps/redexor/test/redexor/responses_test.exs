defmodule Redexor.ResponsesTest do
  ...
  describe "rdx_routes" do
    ...
    test "create_response/2 validates TEXT type", %{user: user, rdx_route: rdx_route} do
      assert {:error, %Ecto.Changeset{}} = ResponseTemplates.create_response(user, rdx_route, %{
        code: 403,
        type: "TEXT",
        title: "response_template title"
      })
      ...
      {:ok, _response} = ResponseTemplates.create_response(user, rdx_route, %{
        code: 403,
        type: "TEXT",
        text_body: "hello world!",
        title: "response_template title"
    ...
    test "delete_response/2 deletes proper response_template", %{user: user, rdx_route: rdx_route} do
      {:ok, first_response} = ResponseTemplates.create_response(user, rdx_route, %{
        code: 403,
        type: "TEXT",
      ...
      {:ok, %{id: second_response_id} = second_response} = ResponseTemplates.create_response(user, rdx_route, %{
        code: 404,
        type: "TEXT",
        text_body: "second_response",
        title: "second_response"
      })
      assert {:ok, %ResponseTemplate{id: ^second_response_id}} = ResponseTemplates.delete_response(user, second_response)
    ...
    test "set_selected/2 selects given response_template", %{user: user, rdx_route: rdx_route} do
      ...
      assert {:ok, %ResponseTemplate{id: ^second_response_id, selected: true}} = ResponseTemplates.set_selected(user, second_response)
      assert %ResponseTemplate{selected: false} = ResponseTemplates.get_response(user, first_response.id)
    end
  end
end
