defmodule Redexor.ResponsesTest do
  ...
  describe "rdx_routes" do
    ...
    test "given a route without responses, a created response is listed for the route", %{user: user, rdx_route: rdx_route} do
      ...
      assert Responses.list_responses(user, rdx_route.id) == [response]
    end

    test "create_response/2 validates TEXT type", %{user: user, rdx_route: rdx_route} do
      assert {:error, %Ecto.Changeset{}} = Responses.create_response(user, rdx_route, %{
        code: 403,
        type: "TEXT",
        title: "response title"
      })
      ...
      {:ok, _response} = Responses.create_response(user, rdx_route, %{
        code: 403,
        type: "TEXT",
        text_body: "hello world!",
        title: "response title"
    ...
    test "delete_response/2 deletes proper response", %{user: user, rdx_route: rdx_route} do
      {:ok, first_response} = Responses.create_response(user, rdx_route, %{
        code: 403,
        type: "TEXT",
      ...
      {:ok, %{id: second_response_id} = second_response} = Responses.create_response(user, rdx_route, %{
        code: 404,
        type: "TEXT",
      ...
      assert Responses.list_responses(user, rdx_route.id) == [first_response]
    end

    test "set_selected/2 selects given response", %{user: user, rdx_route: rdx_route} do
      {:ok, first_response} = Responses.create_response(user, rdx_route, %{
        code: 403,
        type: "TEXT",
        text_body: "first_response",
