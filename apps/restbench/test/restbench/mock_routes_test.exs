defmodule Restbench.MockRoutesTest do
  ...
  describe "mock_routes" do
    ...
    test "get_mock_route!/1 returns the mock_route with given id", %{user: user, mock_server: mock_server} do
      mock_route = mock_route_fixture(user, mock_server)
      assert MockRoutes.get_mock_route(user, mock_route.id) == mock_route
    end

    test "create_mock_route/3 with valid data creates a mock_route", %{user: user, mock_server: mock_server} do
      valid_attrs = %{enabled: true, method: "GET", path: "some path", title: "some title"}

      assert {:ok, %MockRoute{} = mock_route} = MockRoutes.create_mock_route(user, mock_server, valid_attrs)
    ...
    test "create_mock_route/1 with invalid data returns error changeset", %{user: user, mock_server: mock_server} do
      assert {:error, %Ecto.Changeset{}} = MockRoutes.create_mock_route(user, mock_server, @invalid_attrs)
    end

    test "update_mock_route/2 with valid data updates the mock_route", %{user: user, mock_server: mock_server} do
      mock_route = mock_route_fixture(user, mock_server)
      update_attrs = %{enabled: false, method: "POST", path: "some updated path", title: "some updated title"}

      assert {:ok, %MockRoute{} = mock_route} = MockRoutes.update_mock_route(user, mock_route, update_attrs)
    ...
    test "update_mock_route/2 with invalid data returns error changeset", %{user: user, mock_server: mock_server} do
      mock_route = mock_route_fixture(user, mock_server)
      assert {:error, %Ecto.Changeset{}} = MockRoutes.update_mock_route(user, mock_route, @invalid_attrs)
    ...
    test "change_mock_route/2 returns a mock_route changeset", %{user: user, mock_server: mock_server} do
      mock_route = mock_route_fixture(user, mock_server)
